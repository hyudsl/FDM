...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        self.sample_unit = config['solver']['sample_unit']
        self.sample_iterations = config['solver']['sample_iterations']
        if self.sample_unit == 'epoch':
            self.sample_iterations = self.dataloader['train_iterations'] * self.sample_iterations
    ...
    def sample(self, batch, phase='train', step_type='iteration', batch_idx=0):
        tic = time.time()
        if phase != 'fid_test':
        ...
        with torch.no_grad(): 
            ...
            for k, v in samples.items():
                ...
                if phase == 'fid_test':
                    save_path = os.path.join(save_dir, 'bidx{:010d}_rank{}'.format(batch_idx, get_rank()))
                else:
                ...
                if torch.is_tensor(v) and v.dim() == 4 and v.shape[1] in [1, 3]: # image
                    if phase == 'fid_test':
                        for idx, im in enumerate(v):
                            ...
                            # save images
                            im = im.permute(1, 2, 0).to('cpu').numpy()
                            im = Image.fromarray(im)

                            im.save(save_path + '_' + str(idx) + '.png')
                    else:
                        im = v
                        im = im.to(torch.uint8)
                        ...
                        self.logger.add_images(tag='{}/{}'.format(phase, k), img_tensor=im, global_step=step, dataformats='NCHW')
        ...
        for epoch in range(start_epoch, self.max_epochs):
            ...
            if train:
                ...
                if (epoch+1) % 20 == 0:
                    self.model.eval()
                    if 'validation_loader' in self.dataloader:
                        for batch_idx, batch_val in enumerate(self.dataloader['validation_loader']):
                            self.sample(batch_val, phase='fid_test', step_type='iteration', batch_idx=batch_idx)
                            if self.logger is not None and (batch_idx+1) % self.args.log_frequency == 0:
                                self.logger.log_info('Test : iter {}/{}'.format(batch_idx, self.dataloader['validation_iterations']))
                    self.model.train()
