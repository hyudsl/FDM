...
from image_synthesis.modeling.modules.edge_connect.image_gradient_loss import ImageGradientLoss
...
class StyleLoss(nn.Module):
    ...
    def __call__(self, x, y, type='normal'):
        # Compute features
        x_vgg, y_vgg = self.vgg(x), self.vgg(y)
        
        # Compute loss
        style_loss = 0.0
        if type == 'shallow' or type == 'normal':
            style_loss += self.criterion(self.compute_gram(x_vgg['relu2_2']), self.compute_gram(y_vgg['relu2_2']))
        if type == 'normal':
            style_loss += self.criterion(self.compute_gram(x_vgg['relu3_4']), self.compute_gram(y_vgg['relu3_4']))
            style_loss += self.criterion(self.compute_gram(x_vgg['relu4_4']), self.compute_gram(y_vgg['relu4_4']))
        if type == 'deep' or type =='normal':
...
class EdgeConnectLoss(nn.Module):
    def __init__(self,
        ...
        if self.g_gradient_loss_weight > 0.0:
    ...
    def forward(self, 
        ...
        out = {}
        if name == 'fdm':
            out['loss'] = self.l1_loss(reconstruction, image) * self.g_rec_loss_weight
            ...
            gt_feature = other_input['gt_feature']
            lost_feature = other_input['lost_feature']
            lip_loss = self.l1_loss(lost_feature, gt_feature) * self.g_rec_loss_weight
            out['fdm_loss'] = lip_loss
            gen_loss += lip_loss
