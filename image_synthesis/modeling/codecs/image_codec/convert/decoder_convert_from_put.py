...
from image_synthesis.modeling.modules.original_module_backup import ConvResBlock_ORG
...
class PatchDecoder(nn.Module):
    ...
    def convert(self):
        ...
        for i in range(len(self.res_layers)):
            self._res_layers[i].conv[0] = self.res_layers[i].conv[1]
            self._res_layers[i].conv[2] = self.res_layers[i].conv[3]
